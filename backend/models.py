...
class Reviews(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    netid = db.Column(db.String(80), db.ForeignKey('users.netid'))
    photographer_netid = db.Column(db.String(80), db.ForeignKey('photographers.photographer_netid'))
...
class Equipment(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    netid = db.Column(db.String(80), db.ForeignKey('photographers.photographer_netid'))
    equip = db.Column(db.String(80), unique=True)
...
class Portfolio(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    netid = db.Column(db.String(80), db.ForeignKey('photographers.photographer_netid'))
    picture = db.Column(db.String(255), unique=True)
